...
function functionPopContainer() {
    ...
    specie_name_botanico.innerHTML = data[id_hover[0]].nome_en;
}
...
sp_mapa_botanico.forEach(function (c) {
    c.addEventListener("click", function () {
        ...
        exposicao_json(id_clicado[0] - 1); //ALTEREI exposicao_json_EN
...
sp_mapa_botanico.forEach(function (a, i) {
    ...
    d3.json("/assets/data/dados_inside_out_v6.json").then(d => {
        ...
        let nomeComum = data[id_hover[0]].nome_en;
    ...
    textLegenda.addEventListener("click", function () {
        ...
        exposicao_json(textLegenda.id - 1); //ALTEREI exposicao_json_EN
        specie_container.scrollIntoView({
            behavior: 'smooth'
        })
    })
...
function legendaFunction() {
    if (legedanOpen == true) {
        legendaMapaTitle.innerHTML = "Species +";
        legedanOpen = false;
...
//BOTÃO PT ----------------------------------------------------------------------------------------------------------------------
let buttonPT = document.getElementById("botaoPT_2");
